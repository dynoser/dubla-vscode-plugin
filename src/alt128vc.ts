...
export default class alt128vc {
    ...
    public static encode(data: string): string {
        ...
        if (alt128vc.gzEnabled && (canZlib || canPako)) {
        ...
        return vc128.encodeUint8Arr(outUi8Arr);
    }

    public static decode(data: string): string {
        const outUi8Arr = alt128vc.decodeToUint8Arr(data);
        const textDecoder = new TextDecoder('utf-8');
        return textDecoder.decode(outUi8Arr);        
    }

    public static decodeToUint8Arr(data: string): Uint8Array {
        const uint8arr: Uint8Array = vc128.decodeToUint8Arr(data);
